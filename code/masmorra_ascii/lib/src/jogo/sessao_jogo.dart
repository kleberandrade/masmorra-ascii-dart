...
import '../economia/loja.dart';
import '../modelos/fabrica_inimigo.dart';
import '../modelos/item.dart';
import '../modelos/jogador.dart';
import '../modelos/sala.dart';
import '../parse/comando_jogo.dart';
import '../parse/parseador.dart';
import '../persistencia/gerenciador_salve.dart';
import '../ui/banner.dart';
import '../mundo/mapa_masmorra.dart';
import '../mundo/mundo_texto.dart';
import '../mundo/dados_mundo.dart';

/// Caps. 7+ — menu, exploração, loja, masmorra, save/load, log em stream.
class SessaoJogo {
  SessaoJogo({this.sementeMasmorra = 42, this.caminhoSave = 'masmorra_save.json'});

  final int sementeMasmorra;
  final String caminhoSave;

  late Jogador _jogador;
  late MundoTexto _mundo;
  late List<Arma> _stockLoja;
  ...
  Future<void> run() async {
    ...
    _jogador = Jogador(nome: nome);
    _jogador.inventario.add(
      Arma(id: 'punhal', nome: 'Punhal enferrujado', dano: 2, preco: 5),
    );
    _jogador.equiparArmaPorId('punhal');
    _mundo = criarMundoVila();
    _stockLoja = stockArmasLoja();
    _log('Olá, ${_jogador.nome}. HP ${_jogador.hp} | Ouro ${_jogador.ouro}');
  ...
  String _hud() =>
      '[${_jogador.nome}] HP ${_jogador.hp} | Ouro ${_jogador.ouro} | Sala ${_jogador.salaAtual} | Dano ${_jogador.danoAtual}';

  void _loopExploracao() {
    while (true) {
      final sala = _mundo.obterSala(_jogador.salaAtual);
      print('');
      _log(_hud());
      _log(sala.descricao);
      if (sala.temLoja) {
  ...
  bool _tratarComando(ComandoJogo cmd, Sala sala) {
    switch (cmd) {
      case CmdMenu():
        return false;
      case CmdOlhar():
      ...
      case CmdInventario():
        if (_jogador.inventario.isEmpty) {
          ...
          for (var i = 0; i < _jogador.inventario.length; i++) {
            ...
            final extra = it is Arma ? ' (dano ${it.dano})' : '';
  ...
  void _tentarCombateNaSala() {
    final s = _mundo.obterSala(_jogador.salaAtual);
  ...
  Future<void> _carregar() async {
    try {
      ...
      if (data == null) {
        ...
        _log('Carregado, ${data.jogador.nome}.');
      }
    } catch (e) {
